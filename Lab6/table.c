...
static int parti(SET* sp, void** elt, int low, int high)// Big O: O(n).
{
  assert(sp != NULL);
  void* pivot = elt[high];
  int smaller = low - 1;  
...
static void quicksort(SET* sp, void** elt, int low, int high)// Big O: O(n).
{
  assert(sp != NULL);
  if(low < high)
  {
    int partitionIndex = parti(sp, elt, low, high);
